...
def main():
    ...
    model_ft = torch.load("/homes/qzhang/Data/lungwcl/trained_models/ResNet18_lung.pt")
    model_ft.eval()
    print(model_ft)
    ...
    for idx, image_path in enumerate(content):
        ...
        if len(input_image.cpu().detach().numpy().squeeze(1).shape[1:]) == 3:
            for j, (img_slice, camex_slice, cam_slice) in enumerate(zip(input_image.cpu().detach().numpy().squeeze(1), camex_result, cam_result)):
                ...
                if np.isnan(output_cam[:,:,16]).any():
                    print(f"{idx}中CAM存在 NaN 值。")
                if np.isnan(output_camex[:,:,16,-1]).any():
                    print(f"{idx}中CAMEX存在 NaN 值。")
                ...
                plt.imshow(img_slice.squeeze()[:,:,16], cmap="gray")
                plt.contour(mask[0,:,:,16],  levels=[0.5], colors='r', alpha=1, linewidths=3)
                plt.imshow(output_camex[:,:,16,-1], cmap="jet",alpha=0.35)
                plt.title(f"GradCAMEX_-1")
                plt.axis("off")
                plt.savefig(f"/homes/qzhang/Data/lungwcl/GranCAMEx_PNG/{path_acc}/batch{idx}_{j}_cam_261012141516.png")
                plt.close()
    print(TN, TP, FN, FP)
